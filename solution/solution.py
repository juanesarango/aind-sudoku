...
- We use constraint propagation in this case in order to reduce the number of solutions
in each iteration or propagation. For reaching the point where we find the twins we already
took advantage of the previous constraints we apply iteratively to reduce the number of
possible values. Without these previous propagation we wouldn't reach a point where the
values are pairs to match the twins. And finally, the naked twins approach allows us to add
one more contraint to our model, reducing our solution space even more to solve it.

How do we apply constraint propagation to solve the diagonal sudoku problem?

- To solve the diagonal sudoku problem, we are adding a new constraint to our model, that are
the diagonals rule of non-repeated numbers. Previously the constraints were only for columns,
rows, adn 3x3 squares. Now with these aditional constraint the propagation is done taking into
consideration one more rule that has to consider before the reduction of the possible values
for each box.
"""
...
def naked_twins(values):
    ...
    for unit in unitlist:
        ...
        twins = [a for i, a in enumerate(twin_candidates) for j, b in enumerate(twin_candidates) if values[a] == values[b] and i != j] # noqa
        # if len(twins):
        #    print('Twins:', twins)
        for twin in twins:
            value = values[twin]
...
main_diagonal = [rows[index] + cols[index] for index in range(0, len(rows))]
second_diagonal = [rows[len(rows) - 1 - index] + cols[index] for index in range(0, len(rows))]
